...
We evaluated 11 state-of-the-art Large Language Models from four major provider categories, as shown in Table~\ref{tab:models}.
...
& GPT-5.2 & 2025 & API \\
\midrule
Google & Gemini 2.5 Flash & 2025 & API \\
& Gemini 2.5 Pro & 2025 & API \\
& Gemini 3 Flash & 2025 (preview) & API \\
& Gemini 3 Pro & 2025 (preview) & API \\
\midrule
Deepseek & Deepseek-Chat & 2024 & API \\
\midrule
Open/Ollama & GLM-4.7 & 2025 & Cloud \\
& Kimi-K2.5 & 2025 & Cloud \\
& Qwen3-VL & 2025 & Cloud \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{(2) Diversity of architectures:} We included both proprietary (OpenAI, Google) and open-weight models (GLM, Kimi, Qwen), spanning dense and mixture-of-experts designs such as DeepSeek-V3 \cite{deepseekv3}.\\
...
\noindent\textbf{Efficiency Metrics.} Beyond accuracy, we introduce three efficiency measures that quantify the cost at which a given level of success is obtained. For a model $m$ evaluated over the task set $T$ (with $|T|=5$), let $s_m \in [0,1]$ denote its success rate, $\bar{c}_m$ its mean tool-call count, and $\bar{d}_m$ its mean completion time in seconds. We define the Tool Efficiency Ratio (TER) and Time Efficiency Ratio (TimeER) as

\begin{equation}
\mathrm{TER}_m = \frac{s_m}{\bar{c}_m}, \qquad
\mathrm{TimeER}_m = \frac{s_m}{\bar{d}_m},
\label{eq:efficiency}
\end{equation}

\noindent where higher values indicate that success is achieved with fewer tool invocations or in less time, respectively. To approximate monetary cost, we estimate the consumed tokens for each run as the product of its tool-call count and a constant average token footprint per call, priced at the provider's per-token rate:

\begin{equation}
\widehat{\mathrm{Cost}}_m = \sum_{t \in T} c_{m,t}\, \cdot\, \tau \,\cdot\, p_m,
\label{eq:cost}
\end{equation}

\noindent where $c_{m,t}$ is the tool-call count of model $m$ on task $t$, $\tau$ is the estimated average number of tokens exchanged per tool call, and $p_m$ is the per-token price of model $m$. Because $\tau$ is held constant across models, $\widehat{\mathrm{Cost}}$ is a relative, order-of-magnitude estimate rather than an exact billing figure; we treat this approximation as a threat to construct validity (Section~\ref{sec:threats}).
...
\noindent We consolidate the threats to the validity of these measurements and their interpretation in Section~\ref{sec:threats}.
